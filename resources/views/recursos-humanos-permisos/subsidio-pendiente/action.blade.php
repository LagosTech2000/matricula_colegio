<div class="d-flex">
    <form action=" {{url('/recursos-humanos-permisos/subsidio-pendiente/'.$data->id)}} " method="post"> 
        {{ method_field('PATCH')}} 
        @csrf  
    ...
    <form id="borrarForm" action=" {{ route('subsidio-pendiente.destroy',$data->id) }}"  id="MensajeBorrar" method="post">
        @method('DELETE')
        @csrf
